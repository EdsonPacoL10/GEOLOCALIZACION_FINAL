...
public class DesaparecidoDAOimpl extends ConexionDB implements DesaparecidoDAO {
    ...
    List<desaparecido> lista=new ArrayList<>();
    String sql;
    sql="select*from desaparecido where id like '%"+texto+"%' or nombre like '%"+texto+"%' or apellido like '%"+texto+"%' or edad like '%"+texto+"%' or sexo like '%"+texto+"%' or descripcion like '%"+texto+"%' or fecha like '%"+texto+"%' ";
        try {
            this.conectar();
        PreparedStatement ps = this.conn.prepareStatement(sql);
        ResultSet rs = ps.executeQuery();
